...
class LoginRn: NSObject, RCTBridgeModule {
  ...
  @objc(createWepinLogin:withAppKey:)
  func createWepinLogin(appId: String, appKey: String) {
    let params = WepinLoginParams(appId: appId, appKey: appKey)
    wepinLogin = WepinLogin(params, sdkType: "react")
  }
  ...
  func initialize(resolve: @escaping RCTPromiseResolveBlock, reject: @escaping RCTPromiseRejectBlock) {
    ...
    Task {
      do {
        let result = try await wepinLogin.initialize()
        resolve(result)
      } catch {
