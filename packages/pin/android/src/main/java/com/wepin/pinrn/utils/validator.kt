...
internal fun validateParameter(value: Any?, paramName: String, promise: Promise): Boolean {
  return when {
    value == null -> {
      promise.reject(WepinReactErrorCode.InvalidParameters.code, "$paramName is null")
      false
    }

    value is String && value.isEmpty() -> {
      promise.reject(WepinReactErrorCode.InvalidParameters.code, "$paramName is empty")
      false
    }
...
internal fun validateReadableMap(
  ...
  for (key in requiredKeys) {
    if (!map.hasKey(key)) {
      promise.reject(WepinReactErrorCode.InvalidParameters.code, "Required key '$key' is missing")
      return false
    }
...
internal fun <T : Enum<T>> validateEnum(
  ...
  if (value == null || value.isEmpty()) {
    promise.reject(WepinReactErrorCode.InvalidParameters.code, "$paramName is null or empty")
    return null
  }
  ...
  return try {
    ...
    promise.reject(WepinReactErrorCode.InvalidParameters.code, "Invalid $paramName value: $value")
    null
  }
}
...
internal fun validateActivity(context: Activity?, promise: Promise): Activity? {
  if (context == null) {
    promise.reject(WepinReactErrorCode.NotActivity.code, "Activity context is invalid")
    return null
  }
  return context
}
