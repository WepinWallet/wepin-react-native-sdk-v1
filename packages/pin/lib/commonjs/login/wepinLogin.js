...
class WepinLogin {
  ...
  async loginWithOauthProvider(params) {
    return await (0, _NativeModuleWepinPin.loginWithOauthProvider)(params);
  }
  async signUpWithEmailAndPassword(email, password, locale) {
    return await (0, _NativeModuleWepinPin.signUpWithEmailAndPassword)(email, password, locale);
  }
  async loginWithEmailAndPassword(email, password) {
    return await (0, _NativeModuleWepinPin.loginWithEmailAndPassword)(email, password);
  }
  async loginWithIdToken(params) {
    return await (0, _NativeModuleWepinPin.loginWithIdToken)(params);
  }
  async loginWithAccessToken(params) {
    return await (0, _NativeModuleWepinPin.loginWithAccessToken)(params);
  }
  async getRefreshFirebaseToken(prevToken) {
    return await (0, _NativeModuleWepinPin.getRefreshFirebaseToken)(prevToken);
  }
  ...
  async loginWepin(param) {
    return await (0, _NativeModuleWepinPin.loginWepin)(param);
  }
  async getCurrentWepinUser() {
    return await (0, _NativeModuleWepinPin.getCurrentWepinUser)();
  }
  async logoutWepin() {
    return await (0, _NativeModuleWepinPin.logout)();
  }
}
